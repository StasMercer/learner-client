...
export default function generatePdf(firstName, lastName, courseName, teacherName, right, wrong) {
    let docDefinition = {
        ...
        content: [
            ...
            {
                text: 'Даний сертифікат підтверджує що '+firstName + ' ' + lastName + ' закінчив курс',
                style: 'header'
            },
            {
                text: courseName,
            ...
            'З результатом: '+right+' з '+(right+wrong)+ ' можливих балів',
            'Дата: '+ moment(new Date().toISOString()).format('LL'),
            'Вчитель: ' + teacherName
        ],
        styles:{
